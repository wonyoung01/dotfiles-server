...
set_idle_title() {
  if [[ -n "$SSH_CONNECTION" ]]; then
    ...
    if [[ -n "$SERVERNAME" ]]; then
      ...
      local parts=(${(s: :)SSH_CONNECTION})
      host="%n@${parts[3]}"
    fi
    print -Pn "\e]0;$host: %2~\a"
  else
    print -Pn "\e]0;%2~\a"
  fi
}
...
set_running_title() {
  local cmd=${2%% *}   # only program name (python, node, etc.)
  if [[ -n "$SSH_CONNECTION" ]]; then
    local host
    if [[ -n "$SERVERNAME" ]]; then
      host="$SERVERNAME"
    ...
    print -Pn "\e]0;$host: %2~ — $cmd\a"
  else
    print -Pn "\e]0;%2~ — $cmd\a"
  fi
}
...
# Source all .zsh files in ~/.config/zsh/ if the directory exists
for file in ~/.config/zsh/*.zsh; do
  [[ "$(basename "$file")" == "tools.zsh" ]] && continue
  source "$file"
done
...
# 	rails git textmate ruby lighthouse zsh-syntax-highlight zsh-autosuggestions
# 	)
